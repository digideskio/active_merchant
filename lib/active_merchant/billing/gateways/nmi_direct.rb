...
module ActiveMerchant #:nodoc:
  module Billing #:nodoc:
    class NmiDirectGateway < Gateway
      self.live_url = self.test_url = 'https://secure.nmi.com/api/transact.php'
      ...
      VAULT = {
        ...
        :unstore  => 'delete_customer'
      }

      self.supported_countries = ['US']
      self.supported_cardtypes = [:visa, :master, :american_express, :discover]
      self.homepage_url = 'http://nmi.com/'
      self.display_name = 'NMI Direct'
      ...
      # To create a charge on a card, call
      #
      ...
      def void(authorization, options = {})
        capture_void_or_refund_template(:void, nil, authorization, options)
      end
      ...
      def store(credit_card, options = {})
        sale_authorization_or_credit_template(:store, nil, credit_card, options)
      end

      def update(customer_id, credit_card, options = {})
      ...
      def add_customer_data(post, options)
        ...
        post[:customer_vault_id]  = options[:customer]
      end
      ...
      def commit(trx_type, money, post)
        ...
        authorization = TRANSACTIONS.has_key?(trx_type) ? response[:transactionid] : response[:customer_vault_id]
